...
function render(){
  ...
  function edgeOpacity(d){
    ...
    const nearBoost = dist <= 95 ? 1.0 : dist <= 180 ? 0.75 : dist <= 300 ? 0.45 : 0.22;
    const base = d.type === 'paper_sim_cross' ? 0.26 : 0.11;
    const weightBoost = Math.min(0.12, w * 0.01);
    return Math.max(0.02, Math.min(0.32, (base + weightBoost) * nearBoost));
  }

  function edgeWidth(d){
    const w = Number(d.weight || 0);
    return d.type === 'paper_sim_cross' ? Math.min(1.0, 0.5 + w * 0.03) : Math.min(0.62, 0.36 + w * 0.02);
  }
  ...
  for (const [cid, plist] of papersByCat) {
    ...
    const use = contours.slice(0, Math.min(3, contours.length));
    use.forEach((ct, i) => {
      blobLayer.append('path')
        .attr('d', geoPath(ct))
        .attr('fill', color)
        .attr('fill-opacity', 0.010 - i * 0.0028)
  ...
  paperGlow.selectAll('circle.paper-glow').data(papers).join('circle')
    ...
    .attr('cx', d => d.x)
    .attr('cy', d => d.y)
    .attr('r', d => Math.max(18, d.r * 4.5))
    .attr('fill', d => d.color)
    .attr('fill-opacity', 0.016)
    .attr('filter', 'url(#paper-glow-blur)');
...
const trayOnLoad = document.getElementById('charts-tray');
if (trayOnLoad && trayOnLoad.classList.contains('active') && !chartsInited) {
  initCharts();
  chartsInited = true;
}
