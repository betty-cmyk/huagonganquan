...
function render(){
  ...
  const DY = D * 0.866;
  const hexGrid = [
    {cx: -D, cy: -DY}, {cx: 0, cy: -DY}, {cx: D, cy: -DY},
    {cx: -1.5*D, cy: 0}, {cx: -0.5*D, cy: 0}, {cx: 0.5*D, cy: 0}, {cx: 1.5*D, cy: 0},
    {cx: -D, cy: DY}, {cx: 0, cy: DY}, {cx: D, cy: DY}
  ];

  categories.forEach((c, i)=>{
    const pos = hexGrid[i % hexGrid.length];
    c.fx = W/2 + pos.cx;
    c.fy = H/2 + pos.cy;
    c.x = c.fx; c.y = c.fy;
  });
  ...
  papers.forEach(p=>{
    ...
    const rr = 24 + ((p.id * 37) % 180);
    p.x = c.fx + Math.cos(a) * rr;
    p.y = c.fy + Math.sin(a) * rr;
  });

  const sim = d3.forceSimulation(G.nodes)
    .force('link', d3.forceLink(G.edges).id(d=>d.id).distance(d=>d.type==='cat_cat'?150:40).strength(d=>d.type==='cat_cat'?0.1:0.4))
    .force('charge', d3.forceManyBody().strength(d=>d.type==='paper'?-8:0))
    .force('x', d3.forceX(d=>d.type==='category' ? d.fx : (catByKey.get(d.primary_category)?.fx || W/2)).strength(d=>d.type==='category'?1:0.35))
    .force('y', d3.forceY(d=>d.type==='category' ? d.fy : (catByKey.get(d.primary_category)?.fy || H/2)).strength(d=>d.type==='category'?1:0.35))
    .force('collision', d3.forceCollide().radius(d=>d.type==='paper' ? d.r + 2.5 : 0));

  for(let i=0;i<550;i++) sim.tick();
  sim.stop();
