...
function render(){
  ...
  if (defs.select('#cloud-blur').empty()) {
    defs.append('filter')
      .attr('id', 'cloud-blur')
      .attr('x', '-40%')
      .attr('y', '-40%')
  ...
  cloudBg.selectAll('circle.cloud-outer').data(categories).join('circle')
    .attr('class', 'cloud-outer')
    .attr('cx', d => d.x)
    .attr('cy', d => d.y)
    .attr('r', d => Math.max(S * 5.8, Math.sqrt((d.count || 1)) * S * 2.7))
    .attr('fill', d => d.color)
    .attr('fill-opacity', 0.055)
    .attr('filter', 'url(#cloud-blur)');

  cloudBg.selectAll('circle.cloud-inner').data(categories).join('circle')
    .attr('class', 'cloud-inner')
    .attr('cx', d => d.x)
    .attr('cy', d => d.y)
    .attr('r', d => Math.max(S * 3.2, Math.sqrt((d.count || 1)) * S * 1.8))
    .attr('fill', d => d.color)
