...
function render(){
  ...
  edgeGlow.selectAll('line.sim-glow').data(relEdges).join('line')
    .attr('class', 'sim-glow')
    .attr('x1', d => nodeById.get(d.source)?.x ?? 0)
    .attr('y1', d => nodeById.get(d.source)?.y ?? 0)
    .attr('x2', d => nodeById.get(d.target)?.x ?? 0)
    .attr('y2', d => nodeById.get(d.target)?.y ?? 0)
    .attr('stroke', d => d.type === 'paper_sim_cross' ? '#58a6ff' : '#8b949e')
    .attr('stroke-width', d => d.type === 'paper_sim_cross' ? 3.2 : 2.1)
    .attr('stroke-opacity', d => d.type === 'paper_sim_cross' ? 0.26 : 0.16)
