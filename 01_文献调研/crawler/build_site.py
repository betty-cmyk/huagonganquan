...
function initCharts(){
  ...
  const labelArc = d3.arc().innerRadius(ch/2.2).outerRadius(ch/2.2);
  csvg.selectAll('path').data(pie(cats)).join('path').attr('d',arc).attr('fill',d=>d.data.color).attr('stroke','#161b22').attr('stroke-width',1);
  csvg.selectAll('.pie-label').data(pie(cats)).join('text').attr('class','pie-label').attr('transform',d=>`translate(${labelArc.centroid(d)})`).text(d=>`${d.data.label.replace(/^.*_/,'')} (${d.data.count})`).attr('text-anchor','middle').attr('fill','#c9d1d9').style('font-size','9px');
